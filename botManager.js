...
    async fetchMembersForBroadcast(client, guildId, processedMembers) {
...
                    membersToMessage = Array.from(fetchedMembers.values())
                        .filter(m => !m.user.bot && !processedMembers.has(m.id));
                } else {
                    const seen = new Set();
...
                    this.logToDashboard(`Rate limited while fetching members. Waiting ${retryAfter}s...`, null, true);
                    await new Promise(resolve => setTimeout(resolve, (retryAfter * 1000) + 1000));
                } else {
                    this.logToDashboard(`Error fetching members: ${err.message}`, null, true);
                    fetchSuccess = true;
                }
            }
        }
        return membersToMessage;
    }
...
            await Bot.findOneAndUpdate({ token }, { status: 'offline' }, { upsert: true });
        }
    }
...
        const botsData = await Bot.find({ status: 'active' }).sort({ createdAt: 1 });
...
                        const guild = await client.guilds.fetch(guildId);
                        const fetchedMembers = await guild.members.fetch();
                        membersToMessage = Array.from(fetchedMembers.values()).filter(m =>
                            !m.user.bot &&
                            !processedMembers.has(m.id) 
                        );
                    } else {
...
            let burstCounter = 0;
            let allMembersInGuildFinished = true;
            let botBanned = false;

            for (const member of membersToMessage) {
                if (this.status !== 'running' || sentCount >= this.currentBroadcast.totalTarget || botSentThisRound >= MESSAGE_QUOTA_PER_BOT) {
                    if (botSentThisRound < MESSAGE_QUOTA_PER_BOT && sentCount < this.currentBroadcast.totalTarget) {
                        allMembersInGuildFinished = false;
                    }
                    break;
                }
...
                    await Bot.findByIdAndUpdate(currentBotData._id, { $inc: { successCount: 1 }, lastUsed: new Date() });
...
                    this.currentBroadcast.logs.push({ botId: currentBotData._id, message: `Failed: ${member.user.tag} (${error.message})`, isError: true });
                    await this.currentBroadcast.save();

                    this.emitProgressUpdate();

                    if (error.status === 401 || error.message.includes('flagged') || error.message.includes('anti-spam')) {
                        await Bot.findByIdAndUpdate(currentBotData._id, { status: 'banned' });
...
                this.logToDashboard(`Bot #${currentBotIndex + 1} finalized its ${MESSAGE_QUOTA_PER_BOT} messages quota.`, currentBotData._id);
                currentBotIndex++;
            } else if (sentCount >= this.currentBroadcast.totalTarget) {
                break;
            } else {
...
            this.status = 'stopped';
        } else {
            completionMsg = `⚠️ Finished: All available members have been messaged. Total: ${sentCount}`;
...
        const results = [];
        const botsData = await Bot.find({ status: 'active' });
...
    async resetStats() {
        try {
            await Bot.updateMany({}, { successCount: 0 });
